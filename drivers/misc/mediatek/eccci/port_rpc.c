...
static int get_md_adc_val(unsigned int num)
{
	...
	//ES2-2691 RFT: Modem crash while LTE testing
	//Fix begin by kevin.tian@fih-foxconn.com
	//The fix is only for ES2/CO2 project
	if(num == 0xd)
	{
	  extern char *saved_command_line;
	  const char* patten = "moduleadc=";
	  int module = 0;
	  char * module_str = strnstr(saved_command_line,patten,4096);
	  if(module_str!= NULL)
	  {
	    char * cptr = module_str+strlen(patten);
	    while(*cptr>='0'&&*cptr<='9')
	    {
	      module = module*10+ (*cptr)-'0';
	      cptr++;
	    }
	    ret = 0;
	    val  = module;
	  }
	  CCCI_BOOTUP_LOG(0, RPC, "adc val =%d", val);
	}
	//Fix ES2-2691 end
	if (ret == 0)
		return val;
