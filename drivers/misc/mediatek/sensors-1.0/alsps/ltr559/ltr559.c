...
#define CALI_TIMES  3
#define PROBE_CALI_TIMES  3
#define AUTO_CALI_DELAY 50
#define PROBE_CALI_DELAY 60
...
static bool turning_log_enable = false;
...
static int ltr559_ps_enable(struct i2c_client *client, int enable)
{
	...
	{
		if(!is_ftm_mode){//if in ftm mode,do not dynamic cal
#ifdef GN_MTK_BSP_PS_DYNAMIC_CALI
...
static int ltr559_dynamic_calibrate(int count_t, int delay_ms)
{
	...
	if (noise < 100) {
		...
		ps_thd_val_high = noise + 250;
		ps_thd_val_low  = noise + 190;
	}
	else if (noise < 1000) {
...
static int ltr559_als_read(struct i2c_client *client, u16* data)
{
...
#ifdef CONFIG_MTK_ALSPS_ES2
	if (ratio < 47) {
		luxdata_int = (((15081 * alsval_ch0) + (9400 * alsval_ch1)) / als_gainrange) / 1000;
	}
	else if ((ratio < 64) && (ratio >= 47)) {
		luxdata_int = (((21392 * alsval_ch0) - (9774 * alsval_ch1)) / als_gainrange) / 1000;
	}
	else if ((ratio < 85) && (ratio >= 64)) {
		luxdata_int = (((991 * alsval_ch0) + (401 * alsval_ch1)) / als_gainrange) / 1000;
	}
	else {
		luxdata_int = 0;
	}
	...
	if (ratio < 45) {
		...
		luxdata_int = (((21400 * alsval_ch0) + (9800 * alsval_ch1)) / als_gainrange) / 1000;
	}
	else if ((ratio < 85) && (ratio >= 64)) {
		luxdata_int = (((991 * alsval_ch0) + (401 * alsval_ch1)) / als_gainrange) / 1000;
	}
...
#ifndef _USER_BUILD_
	if (turning_log_enable)
		APS_LOG("ltr559_als_read: als_value_lux = %d\n", luxdata_int);
#endif
out:
	*data = luxdata_int;
...
/*----------------------------------------------------------------------------*/
static ssize_t ltr559_store_dbglog(struct device_driver *ddri, const char *buf, size_t count)
{
	int debug_log;
	...
	if(1 != sscanf(buf, "%d", &debug_log))
	{
		APS_ERR("target_lux: '%d', length = %zu\n", debug_log, count);
		return -1;
	}
	APS_LOG("debug_log = %d", debug_log);
	APS_LOG("ltr559_obj->debug_log (%s)!!\n", debug_log ? ("true") : ("false"));

	if(debug_log == 1) {
		turning_log_enable = true;
	}else {
		turning_log_enable = false;
	}
	return count;
}
/*--------------------------------------------------------*/
static ssize_t ltr559_show_dbglog(struct device_driver *ddri, char *buf)
{
	int len;
	if(!ltr559_obj)
	{
	...
	APS_LOG("ltr559_obj->debug_log(%d)\n", turning_log_enable);
	return len;
}
...
static DRIVER_ATTR(dbglog,  S_IWUSR | S_IRUGO, ltr559_show_dbglog, ltr559_store_dbglog);
...
static void ltr559_eint_work(struct work_struct *work)
{
	...
	else
	{
		...
		if (just_enable && intr_flag_value && ENABLE_EINT_CALIB) {
...
#endif
			if((intr_flag_value == 0) && ENABLE_EINT_CALIB)
			{  //far event
				obj->ps = ltr559_get_ps_slope_array(work);
...
/*-----------------------------------------------------------------------------------*/
static int ltr599_als_factory_enable_sensor(bool enable_disable, int64_t sample_periods_ms)
{
	int err = 0;
...
static int ltr599_als_factory_get_data(int32_t *data)
{
	int status;

	return als_get_data(data, &status);
}
static int ltr599_als_factory_get_raw_data(int32_t *data)
{
	int status;

	return als_get_data(data, &status);
}
static int ltr599_als_factory_enable_calibration(void)
{
	return 0;
}
static int ltr599_als_factory_clear_cali(void)
{
	return 0;
}
static int ltr599_als_factory_set_cali(int32_t offset)
{
	APS_ERR("[FIH] Entry ltr559_als_factory_set_cali\n");

	return 0;
}
static int ltr599_als_factory_get_cali(int32_t *offset)
{
	return 0;
}
static int ltr599_ps_factory_enable_sensor(bool enable_disable, int64_t sample_periods_ms)
{
	int err = 0;
...
static int ltr599_ps_factory_get_data(int32_t *data)
{
	int err = 0, status = 0;
...
static int ltr599_ps_factory_get_raw_data(int32_t *data)
{
	int err = 0;
	if(!ltr559_obj)
	{
...
static int ltr599_ps_factory_enable_calibration(void)
{
	return 0;
}
static int ltr599_ps_factory_clear_cali(void)
{
	return 0;
}
static int ltr599_ps_factory_set_cali(int32_t als_cali_data[2])
{
	APS_ERR("[FIH] Entry ltr559_ps_factory_set_cali\n");
	if(!ltr559_obj){
		APS_ERR("ltr559_obj is null!!\n");
		return -1;
	}
	ltr559_obj->als_target = als_cali_data[0];
	ltr559_obj->als_cali = als_cali_data[1];
	ltr559_update_als_cali_slope();
	return 0;
}
static int ltr599_ps_factory_get_cali(int32_t *offset)
{
	return 0;
}
static int ltr599_ps_factory_set_threashold(int32_t threshold[2])
{
	return 0;
}
static int ltr599_ps_factory_get_threashold(int32_t threshold[2])
{
	return 0;
}

static struct alsps_factory_fops ltr599_factory_fops = {
	.als_enable_sensor = ltr599_als_factory_enable_sensor,
	.als_get_data = ltr599_als_factory_get_data,
	.als_get_raw_data = ltr599_als_factory_get_raw_data,
	.als_enable_calibration = ltr599_als_factory_enable_calibration,
	.als_clear_cali = ltr599_als_factory_clear_cali,
	.als_set_cali = ltr599_als_factory_set_cali,
	.als_get_cali = ltr599_als_factory_get_cali,

	.ps_enable_sensor = ltr599_ps_factory_enable_sensor,
	.ps_get_data = ltr599_ps_factory_get_data,
	.ps_get_raw_data = ltr599_ps_factory_get_raw_data,
	.ps_enable_calibration = ltr599_ps_factory_enable_calibration,
	.ps_clear_cali = ltr599_ps_factory_clear_cali,
	.ps_set_cali = ltr599_ps_factory_set_cali,
	.ps_get_cali = ltr599_ps_factory_get_cali,
	.ps_set_threashold = ltr599_ps_factory_set_threashold,
	.ps_get_threashold = ltr599_ps_factory_get_threashold,
};

static struct alsps_factory_public ltr599_factory_device = {
	.gain = 1,
	.sensitivity = 1,
	.fops = &ltr599_factory_fops,
};
/*-----------------------------------i2c operations----------------------------------*/
static int ltr559_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	err = alsps_factory_device_register(&ltr599_factory_device);
	if (err) {
		APS_ERR("ltr599_device register failed\n");
	...
	/*----------------ltr599 attribute file for debug------------------------------*/
