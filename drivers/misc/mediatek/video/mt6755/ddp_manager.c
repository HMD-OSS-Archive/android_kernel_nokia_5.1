...
int dpmgr_path_user_cmd(disp_path_handle dp_handle, int msg, unsigned long arg, void *cmdqhandle)
{
	...
	case DISP_IOCTL_READ_SW_REG:
		...
		break;
	case DISP_IOCTL_DCS_CMD:
	{
		if (ddp_modules_driver[DISP_MODULE_DSI0]->ioctl != NULL)
			ret = ddp_modules_driver[DISP_MODULE_DSI0]->ioctl(DISP_MODULE_DSI0, cmdqhandle ,DDP_DSI_DCS_CMD, (void *)arg);
