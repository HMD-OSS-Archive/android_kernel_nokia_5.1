...
#include <linux/hrtimer.h>//Added to press hook twice or three times
...
----------------------------------------------------------------------*/
#define JUST_INPUT_NO_SWITCH 1   /* 0:only use input;   1: input & switch both */
...
#endif
	mutex_lock(&accdet_eint_irq_sync_mutex);
	if ((1 == eint_accdet_sync_flag) && (accdet_status != PLUG_OUT)) {//CO2-2837
		...
		ACCDET_DEBUG("[Accdet] Headset has plugged out don't set accdet state\n");
                send_accdet_status_event(cable_type, 0);//CO2-2837
        }
	mutex_unlock(&accdet_eint_irq_sync_mutex);
