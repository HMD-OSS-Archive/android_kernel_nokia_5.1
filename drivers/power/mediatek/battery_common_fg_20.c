...
#include <linux/nt36xxx_switch_charger.h>
#endif
...
extern void hw_bc11_dcd_release(void);
/* ////////////////////////////////////////////////////////////////////////////// */
...
extern bool fih_is_otg_en(void);
...
static unsigned int ftm_is_charging = KAL_FALSE;
...
/* 20171116 V0.2*/
/* temp range,	mode,	current,	stop,	stop(talk),recharge,recharge(talk),cc2cv,temp change */
...
int First_temp = 0;
extern int rt5081a_ship_mode_enable(bool en);
/* ////////////////////////////////////////////////////////////////////////////// */
				...
				enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_BatteryAverageCurrent:
		val->intval = data->BAT_BatteryAverageCurrent * (-1);
		break;
...
static ssize_t store_Charger_Switch_Value(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	...
	ftm_is_charging = charging_enable;
	battery_charging_control(CHARGING_CMD_ENABLE,&charging_enable);
...
#ifdef CONFIG_DEBUG_CHG
static ssize_t show_Disable_OTP(struct device *dev, struct device_attribute *attr, char *buf)
{

        return sprintf(buf, "%d\n", g_disable_otp);
}

static ssize_t store_Disable_OTP(struct device *dev, struct device_attribute *attr,
                                  const char *buf, size_t size)
{
	int  temp;
...
static DEVICE_ATTR(Disable_OTP, 0664, show_Disable_OTP, store_Disable_OTP);
#endif

static ssize_t store_en_ship_mode(struct device *dev, struct device_attribute *attr,
                                  const char *buf, size_t size)
{
        int  temp;
        ...
        if(temp == 1) {
		rt5081a_ship_mode_enable(1);
        }else {
		rt5081a_ship_mode_enable(0);
        }

        return size;
}

static DEVICE_ATTR(en_ship_mode, 0664, NULL, store_en_ship_mode);
...
void mt_battery_GetBatteryData(void)
{
	...
	signed int ICharging, temperature, temperatureR, temperatureV, temperature1;
	...
	temperature1 = mt_battery_average_method(BATTERY_AVG_TEMP, &batteryTempBuffer[0], temperature,
	...
	if(First_temp == 0)
	{
		...
		for(i=0; i<MODE_NUM; i++) {
			if(BMT_status.temperature>PSE[i][TEMP_L] && BMT_status.temperature<=PSE[i][TEMP_H]) {
	...
	battery_log(BAT_LOG_CRTI,
		"[kernel]AvgVbat %d,bat_vol %d, AvgI %d, I %d, VChr %d, AvgT %d, T %d, ZCV %d, CHR_Type %d, SOC %3d:%3d:%3d, bcct %d:%d, Ichg %d, IBat %d, PSE %d\n",
		...
		BMT_status.IBattery / 10, BMT_status.charge_mode);
}
...
static PMU_STATUS fih_battery_CheckBatteryTemp(void)
{
	...
	if(BMT_status.temperature < PSE[BMT_status.charge_mode][CHANGE_TEMP_L])
		BMT_status.charge_mode++;
	else if(BMT_status.temperature > PSE[BMT_status.charge_mode][CHANGE_TEMP_H])
		BMT_status.charge_mode--;
	else
		no_change = 1;
...
static PMU_STATUS mt_battery_CheckBateryIsValid(void)
{
	PMU_STATUS status = PMU_STATUS_OK;
	if(battery_main.bat_id == -1) {
		BMT_status.bat_charging_state = CHR_ERROR;
		battery_log(BAT_LOG_CRTI, "[BATTERY]Battery is invalid !!\r\n");
		status = PMU_STATUS_FAIL;
	}
	return status;
}
...
#define FIH_FACTORY_CAPACITY 70
#endif
static void ftm_battery_check_charging(void)
{
	if ((get_boot_mode() == ATE_FACTORY_BOOT) || (get_boot_mode() == FACTORY_BOOT)
		|| (get_boot_mode() == FIH_FACTORY_BOOT)) {
		battery_log(BAT_LOG_CRTI, "[BATTERY] enter FTM mode = (%d)\n",get_boot_mode());
		if(KAL_TRUE == ftm_is_charging)
			BMT_status.bat_charging_state = CHR_PRE;
		else if ( BMT_status.UI_SOC2 > FIH_FACTORY_CAPACITY )
			BMT_status.bat_charging_state = CHR_ERROR;
		else
			BMT_status.bat_charging_state = CHR_PRE;
	}
...
static void mt_battery_CheckBatteryStatus(void)
{
	...
	if(mt_battery_CheckBateryIsValid() != PMU_STATUS_OK) {
		BMT_status.bat_charging_state = CHR_ERROR;
		return;
	}
...
static long adc_cali_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case ADC_CHANNEL_READ:
		...
		if (adc_in_data[0] == 0) {
			...
			adc_out_data[0] = battery_meter_get_battery_temperature() * adc_in_data[1] / 10;
...
static int battery_probe(struct platform_device *dev)
{
	...
	batt_init_cust_data();
#ifdef CONFIG_DEBUG_CHG
	temp = get_rtc_spare_charge_value();
	if(temp == 1)
		g_disable_otp = 1;
#endif
	...
	/* For EM */
	{
		...
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_en_ship_mode);
#ifdef CONFIG_DEBUG_CHG
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_Disable_OTP);
#endif
	}
	...
	BMT_status.charge_mode = NORMAL_PHASE;
