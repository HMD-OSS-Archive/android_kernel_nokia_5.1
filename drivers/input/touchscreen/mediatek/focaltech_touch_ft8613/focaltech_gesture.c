...
int fts_create_gesture_sysfs(struct i2c_client *client)
{
    ...
    ret = sysfs_create_group(focal_tp_kobj, &fts_gesture_group);
    if ( ret != 0) {
        FTS_ERROR( "[GESTURE]fts_gesture_mode_group(sysfs) create failed!");
        sysfs_remove_group(focal_tp_kobj, &fts_gesture_group);
        return ret;
    }
    return 0;
}
