...
extern struct kobject *focal_tp_kobj;
...
static int fts_test_save_test_data(char *file_name, char *data_buf, int len)
{
    ...
    sprintf(filepath, "%s%s", FTS_RESULT_FILE_PATH, file_name);
    if (NULL == pfile) {
        pfile = filp_open(filepath, O_TRUNC | O_CREAT | O_RDWR, 0644);
    }
    if (IS_ERR(pfile)) {
...
static int fts_test_entry(char *ini_file_name)
{
    ...
    if (true == fts_test_start()) {
        FTS_TEST_SAVE_INFO_RESULT("Tp test pass\n");
    } else {
        FTS_TEST_SAVE_INFO_RESULT("Tp test failure\n");
    }
...
int fts_test_init(struct i2c_client *client)
{
    ...
    ret = sysfs_create_group(focal_tp_kobj, &fts_test_attribute_group);
    if (0 != ret) {
        FTS_TEST_ERROR( "[focal] %s() - ERROR: sysfs_create_group() failed.",  __func__);
        sysfs_remove_group(focal_tp_kobj, &fts_test_attribute_group);
    } else {
