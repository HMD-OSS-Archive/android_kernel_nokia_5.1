...
static DEVICE_ATTR(tp_fw_version, S_IRUGO | S_IWUSR, fts_tpfwver_show, fts_tpfwver_store);
...
int fts_create_sysfs(struct i2c_client *client)
{
    ...
    ret = sysfs_create_group(focal_tp_kobj, &fts_attribute_group);
    if (ret) {
        FTS_ERROR("[EX]: sysfs_create_group() failed!!");
        sysfs_remove_group(focal_tp_kobj, &fts_attribute_group);
        return -ENOMEM;
    } else {
