...
static int tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    focal_tp_kobj = kobject_create_and_add("android_tp", NULL);
    if(focal_tp_kobj == NULL) {
        FTS_ERROR("%s:focal create kobject is failed\n",__func__);
        return -1;
    }
#if FTS_APK_NODE_EN
